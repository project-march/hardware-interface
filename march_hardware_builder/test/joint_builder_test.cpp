...
TEST_F(JointTest, ValidJointHip)
{
  ...
  march::IMotionCube imc = march::IMotionCube(2, encoder_absolute, encoder_incremental, march::ActuationMode::unknown);
...
TEST_F(JointTest, ValidActuationMode)
{
  ...
  march::Joint expected(march::IMotionCube(
      ...
      march::EncoderAbsolute(16, 22134, 43436, this->joint->limits->lower, this->joint->limits->upper,
                             this->joint->safety->soft_lower_limit, this->joint->safety->soft_upper_limit),
      march::EncoderIncremental(12), march::ActuationMode::position));
  expected.setName("test_joint_hip");
