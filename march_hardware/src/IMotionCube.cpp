...
void IMotionCube::writeInitialSettings(uint8 ecatCycleTime)
{
  ...
  // mode of operation
  success &= sendSDOMessage(slaveIndex, 0x6060, 0, SDOMessageLength::bit_8, this->actuationMode.toModeNumber());

  // position dimension index
  success &= sendSDOMessage(slaveIndex, 0x608A, 0, SDOMessageLength::bit_8, 1);

  // position factor -- scaling factor numerator
  success &= sendSDOMessage(slaveIndex, 0x6093, 1, SDOMessageLength::bit_32, 1);
  // position factor -- scaling factor denominator
  success &= sendSDOMessage(slaveIndex, 0x6093, 2, SDOMessageLength::bit_32, 1);

  // position limit -- min position
  success &= sendSDOMessage(slaveIndex, 0x607D, 1, SDOMessageLength::bit_32, this->encoder.getLowerSoftLimitIU());
  // position limit -- max position
  success &= sendSDOMessage(slaveIndex, 0x607D, 2, SDOMessageLength::bit_32, this->encoder.getUpperSoftLimitIU());

  // Quick stop option
  success &= sendSDOMessage(slaveIndex, 0x605A, 0, SDOMessageLength::bit_16, 6);

  // Quick stop deceleration
  success &= sendSDOMessage(slaveIndex, 0x6085, 0, SDOMessageLength::bit_32, 0x7FFFFFFF);

  // set the ethercat rate of encoder in form x*10^y
  success &= sendSDOMessage(slaveIndex, 0x60C2, 1, SDOMessageLength::bit_8, ecatCycleTime);
  success &= sendSDOMessage(slaveIndex, 0x60C2, 2, SDOMessageLength::bit_8, -3);
...
bool IMotionCube::resetIMotionCube()
{
  ...
  sendSDOMessage(slaveIndex, 0x2080, 0, SDOMessageLength::bit_16, 1);
}
